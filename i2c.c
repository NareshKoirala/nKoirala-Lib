...
int I2C0_Addr(char Address, char IsRead, char WaitForBus)
{
    ...
    {
        if (I2C0_WaitForBus())
            return -1; // bus is busy -- deal with it in main.c
    }
    ...
    */
    ...
    while (!(IIC0_IBSR & IIC0_IBSR_IBIF_MASK))
...
{
    // address the device
    if (I2C0_Addr(0x10, IIC0_WRITE, IIC0_WAIT))
        return -1;
    // send command (write chan, power up all)
    if (dac == DAC_A)
        (void)I2C0_WriteByte(0b00100000, IIC0_NOSTOP); // P18, datasheet
    else if (dac == DAC_B)
        (void)I2C0_WriteByte(0b00100001, IIC0_NOSTOP); // P18, datasheet
    else                                             // assume all channels
        (void)I2C0_WriteByte(0b00101111, IIC0_NOSTOP); // P18, datasheet
    // send msb data (data is 12 bits, oddly, left aligned, P18, datasheet)
    (void)I2C0_WriteByte((unsigned char)(value >> 4), IIC0_NOSTOP); // 0x0123 becomes 0x12
    // send lsb data
    (void)I2C0_WriteByte((unsigned char)(value << 4), IIC0_STOP); // 0x0123 becomes 0x30
    return 0;                                                   // good condition
}
